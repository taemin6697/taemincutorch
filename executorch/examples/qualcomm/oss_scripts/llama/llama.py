...
def _get_config_for_dtype(args, base_config: LLMModelConfig) -> LLMModelConfig:
    """Return config with fp16 overrides (no quantization) when dtype is fp16."""
    dtype = getattr(args, "dtype_override", None) or "fp32"
    ...
    use_fp16_all = (
        dtype == "fp16"
        and vision_quant == "fp16"
        and decoder_quant == "fp16"
        and embedding_quant == "fp16"
    )
    if not use_fp16_all:
        return base_config

    class _FP16ConfigOverride:
        """Wrapper that overrides quant_recipe and vision_encoder for fp16."""
        ...
        def __getattribute__(self, name):
            ...
            if name == "quant_recipe":
                return None
            if name == VISION_ENCODER and hasattr(self._base, VISION_ENCODER):
                return InternVL3EncoderFP16
            return getattr(self._base, name)

    return _FP16ConfigOverride(base_config)


def _use_fp16_compile(args) -> bool:
    """Whether to use fp16 for HTP compilation (no quantization)."""
    dtype = getattr(args, "dtype_override", None) or "fp32"
    vision_quant = getattr(args, "vision_quant", "fp16")
    decoder_quant = getattr(args, "decoder_quant", "fp16")
    embedding_quant = getattr(args, "embedding_quant", "fp16")
...
def compile(
    ...
    config = _get_config_for_dtype(args, decoder_model_config)
    multi_modal_mgr = MultiModalManager(control_args=args, config=config)

    # perform ptq (skipped when config has quant_recipe=None for all components)
    ...
    is_modality = False
    use_fp16 = _use_fp16_compile(args)
...
def _write_foundation_manifest(
    ...
    artifact_root = Path(args.artifact).resolve()
    manifest = build_manifest(
        artifact_root=artifact_root,
        backend="qnn",
        variant=args.decoder_model,
        vision_encoder_pte=Path(encoder_pte_path),
        text_embedding_pte=Path(text_embedding_pte_path),
        text_decoder_pte=Path(text_decoder_pte_path),
        tokenizer_path=Path(runtime_tokenizer_path),
        export={
            "max_seq_len": args.max_seq_len,
            "max_context_len": getattr(args, "max_context_len", args.max_seq_len),
            "model_mode": args.model_mode,
            "soc_model": args.model,
            "dtype": getattr(args, "dtype_override", "fp32"),
        },
        quant={
        ...
        runtime={
            ...
            "qnn_model_mode": args.model_mode,
        },
    )
    write_manifest(manifest, artifact_root / FOUNDATION_MANIFEST_FILENAME)
...
def _build_parser():
    ...
    parser.add_argument(
        ...
        default="fp16",
        choices=["fp16", "16a8w"],
        help="Vision encoder: fp16 (no quant) or 16a8w.",
    )
    parser.add_argument(
        "--decoder_quant",
        default="fp16",
        choices=["fp16", "16a8w", "16a4w", "8a8w"],
        help="Text decoder: fp16 (no quant) or 16a8w/16a4w/8a8w.",
    )
    parser.add_argument(
        "--embedding_quant",
        default="fp16",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Fallback to cpu embedding operator and type of embedding quantization, '<bitwidth>,<groupsize>', e.g., '4,32'. Overridden by --embedding_quant when set.",
    )
...
def export_llama(args) -> None:
    # Sync embedding_quantize from embedding_quant when embedding_quant != fp16
    if args.embedding_quant != "fp16" and args.embedding_quantize is None:
        args.embedding_quantize = args.embedding_quant
