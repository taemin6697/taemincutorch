...
class Modality(Component):
    def __init__(
        ...
        if config := getattr(config, modality, None):
            ...
            # apply dtype_override for fp16 path (align with decoder; avoids conv2d input/bias mismatch)
            if (
                self.control_args.dtype_override == "fp16"
                and config.quant_recipe is None
            ):
                torch_dtype = torch.float16
                self.model = self.model.to(torch_dtype)
                if isinstance(self.example_input, tuple):
                    self.example_input = tuple(
                        t.to(torch_dtype) if t.is_floating_point() else t
                        for t in self.example_input
                    )
                elif self.example_input.is_floating_point():
                    self.example_input = self.example_input.to(torch_dtype)
