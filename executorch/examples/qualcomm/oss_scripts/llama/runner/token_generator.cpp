...
Result<int64_t> TokenGenerator<T>::generate(
  ...
  while (pos < seq_len - 1) {
    ...
    long t_end_ms = time_in_ms();
    if (per_token_timing_cb) {
      int64_t token_idx = pos - start_pos - 1;
      per_token_timing_cb(token_idx, pos, t_start_ms, t_end_ms);
    }

    // data-dependent terminating condition: we have n_eos_ number of EOS
    if (!ignore_eos && eos_ids_->count(cur_token) > 0) {
      printf("\n");
