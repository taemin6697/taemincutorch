...
class KVManager {
 public:
  ...
  /**
   * @brief Copy a range of positions within the KV cache. Handles overlap.
   * Used for shifting cache (e.g. remove prefix from frame, or prepend prefix).
   * @param src_start Source position index
   * @param dst_start Destination position index
   * @param num_positions Number of positions to copy
   */
  void copy_cache_range(
      int64_t src_start,
      int64_t dst_start,
