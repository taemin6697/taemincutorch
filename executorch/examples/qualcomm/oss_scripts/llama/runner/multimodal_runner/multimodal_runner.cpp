...
template <typename T>
uint64_t MultimodalRunner<T>::get_placeholder_token_id() {
  if (module_->method_names()->count("modality_placeholder_token_id") > 0) {
    return module_->get("modality_placeholder_token_id")->toInt();
  }
  return 0;
}

template <typename T>
Error MultimodalRunner<T>::finalize_prefill() {
  if (kv_requantized_) {
    return Error::Ok;
  }
  if (eval_mode_ != EvalMode::kHybrid &&
      eval_mode_ != EvalMode::kLookaheadDecoding) {
    kv_requantized_ = true;
    return Error::Ok;
  }
...
template <typename T>
Error MultimodalRunner<T>::prefill_prefix() {
  ET_CHECK_MSG(is_loaded(), "Call load() before prefill_prefix()");
  ET_CHECK_MSG(!prefix_prefilled_, "prefill_prefix() already called");

  std::vector<uint64_t> prefix_tokens;
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kSmolvlm) {
    prefix_tokens = {1u, 11126u, 42u};
  } else if (
      decoder_model_version_ == MultimodalDecoderModelVersion::kInternvl3) {
    auto res = tokenizer_->encode("<|im_start|>user:\n", /*bos=*/0, /*eos=*/0);
    ET_CHECK_MSG(res.ok(), "Failed to encode InternVL3 prefix");
    prefix_tokens = res.get();
  } else {
    ET_LOG(Error, "prefill_prefix: unsupported model version");
    return Error::NotSupported;
  }

  embedding_processor_->prefill(prefix_tokens);
  const TensorStruct<float>& emb = embedding_processor_->get_prompt_embeddings();

  auto prefill_res =
      prompt_processor_->prefill(emb, cur_pos_, /*dump_logits=*/false, nullptr);
  ET_CHECK_OK_OR_RETURN_ERROR(prefill_res.error());

  cur_pos_ += static_cast<int64_t>(prefix_tokens.size());
  prefix_prefilled_ = true;
  frames_prefilled_ = true;

  ET_LOG(
      Info,
      "Prefix prefilled: %zu tokens, cur_pos=%ld",
      prefix_tokens.size(),
      cur_pos_);
  return Error::Ok;
}
...
template <typename T>
Error MultimodalRunner<T>::prefill_frame(
    const executorch::aten::Tensor& image_hidden_states) {
  ET_CHECK_MSG(prefix_prefilled_, "Call prefill_prefix() before prefill_frame()");

  kv_requantized_ = false;

  image_hidden_states_ =
      std::make_shared<executorch::aten::Tensor>(image_hidden_states);

  int64_t img_seq_len = image_hidden_states.size(1);
  uint64_t placeholder_token_id = get_placeholder_token_id();

  std::vector<uint64_t> prefix_tokens, suffix_tokens;
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kSmolvlm) {
    prefix_tokens = {49189u, 49152u};
    suffix_tokens = {49189u};
  } else if (
      decoder_model_version_ == MultimodalDecoderModelVersion::kInternvl3) {
    int frame_num = prefilled_frame_count_ + 1;
    std::string prefix_str = "Frame" + std::to_string(frame_num) + ": <img>";
    auto prefix_result =
        tokenizer_->encode(prefix_str, /*bos=*/0, /*eos=*/0);
    ET_CHECK_MSG(prefix_result.ok(), "Failed to encode Frame{N}: <img>");
    std::string suffix_str = "</img>\n";
    auto suffix_result =
        tokenizer_->encode(suffix_str, /*bos=*/0, /*eos=*/0);
    ET_CHECK_MSG(suffix_result.ok(), "Failed to encode InternVL frame suffix");
    prefix_tokens = prefix_result.get();
    suffix_tokens = suffix_result.get();
  } else {
    ET_LOG(Error, "prefill_frame: unsupported model version");
    return Error::NotSupported;
  }

  std::vector<uint64_t> all_tokens;
  all_tokens.reserve(prefix_tokens.size() + img_seq_len + suffix_tokens.size());
  all_tokens.insert(all_tokens.end(), prefix_tokens.begin(), prefix_tokens.end());
  for (int64_t i = 0; i < img_seq_len; ++i) {
    all_tokens.push_back(placeholder_token_id);
  }
  all_tokens.insert(all_tokens.end(), suffix_tokens.begin(), suffix_tokens.end());

  embedding_processor_->prefill(all_tokens);
  const TensorStruct<float>& text_embeddings =
      embedding_processor_->get_prompt_embeddings();

  merge_multimodal_embeddings(
      all_tokens, text_embeddings, placeholder_token_id);

  auto prefill_res = prompt_processor_->prefill(
      merged_embeddings_, cur_pos_, /*dump_logits=*/false, nullptr);
  ET_CHECK_OK_OR_RETURN_ERROR(prefill_res.error());

  int64_t total_tokens = static_cast<int64_t>(all_tokens.size());
  cur_pos_ += total_tokens;
  frames_prefilled_ = true;
  ++prefilled_frame_count_;

  ET_LOG(
      Info,
      "Frame %d prefilled: tokens=%ld (img=%ld + wrapper=%ld), cur_pos=%ld",
      prefilled_frame_count_ - 1,
      total_tokens,
      img_seq_len,
      total_tokens - img_seq_len,
      cur_pos_);
  return Error::Ok;
}

template <typename T>
void MultimodalRunner<T>::set_image_hidden_states_batch(
    std::vector<float>&& buffer,
    int64_t total_seq_len,
    int64_t hidden_dim) {
  batch_image_buffer_ = std::move(buffer);
  std::vector<executorch::aten::TensorImpl::SizesType> sizes = {
      1, static_cast<executorch::aten::TensorImpl::SizesType>(total_seq_len),
      static_cast<executorch::aten::TensorImpl::SizesType>(hidden_dim)};
  std::vector<executorch::aten::TensorImpl::DimOrderType> dim_order = {0, 1, 2};
  batch_image_impl_ = std::make_unique<executorch::aten::TensorImpl>(
      executorch::aten::ScalarType::Float,
      sizes.size(),
      sizes.data(),
      batch_image_buffer_.data(),
      dim_order.data());
  image_hidden_states_ =
      std::make_shared<executorch::aten::Tensor>(batch_image_impl_.get());
}

template <typename T>
std::string MultimodalRunner<T>::get_prefix_format_string() const {
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kSmolvlm) {
    return "<|im_start|>User:";
  }
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kInternvl3) {
    return "<|im_start|>user:\n";
  }
  return "";
}
...
template <typename T>
std::string MultimodalRunner<T>::get_frame_format_string(
    int frame_idx, int64_t img_seq_len) const {
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kSmolvlm) {
    std::string s = "<fake_token_around_image><global-img>";
    for (int64_t i = 0; i < img_seq_len; ++i) s += "<image>";
    s += "<fake_token_around_image>";
    return s;
  }
  if (decoder_model_version_ == MultimodalDecoderModelVersion::kInternvl3) {
    int frame_num = frame_idx + 1;
    std::string s = "Frame" + std::to_string(frame_num) + ": <img>";
    for (int64_t i = 0; i < img_seq_len; ++i) s += "<IMG_CONTEXT>";
    s += "</img>\n";
    return s;
  }
  return "";
