...
template <typename T>
void KVManager<T>::copy_cache_range(
    int64_t src_start,
    int64_t dst_start,
    int64_t num_positions) {
  if (num_positions <= 0)
    return;
  const int64_t max_len = metadata_.max_cache_len;
  ET_CHECK_MSG(
      src_start >= 0 && src_start + num_positions <= max_len,
      "copy_cache_range: src range [%ld, %ld) out of bounds [0, %ld)",
      src_start,
      src_start + num_positions,
      max_len);
  ET_CHECK_MSG(
      dst_start >= 0 && dst_start + num_positions <= max_len,
      "copy_cache_range: dst range [%ld, %ld) out of bounds [0, %ld)",
      dst_start,
      dst_start + num_positions,
      max_len);

  for (int layer = 0; layer < metadata_.num_layers; ++layer) {
    // K layout: [num_heads*head_dim][max_cache_len]
    T* k_buf = k_cache_[layer].buffer;
    const int64_t k_row_stride = max_len;
    const int64_t k_row_count =
        metadata_.num_heads * metadata_.head_dim;
    for (int64_t row = 0; row < k_row_count; ++row) {
      T* src = k_buf + row * k_row_stride + src_start;
      T* dst = k_buf + row * k_row_stride + dst_start;
      std::memmove(dst, src, num_positions * sizeof(T));
    }

    // V layout: [num_heads][max_cache_len][head_dim]
    T* v_buf = v_cache_[layer].buffer;
    const int64_t v_pos_stride = metadata_.head_dim;
    const int64_t v_head_stride = max_len * metadata_.head_dim;
    for (int head = 0; head < metadata_.num_heads; ++head) {
      T* src = v_buf + head * v_head_stride + src_start * v_pos_stride;
      T* dst = v_buf + head * v_head_stride + dst_start * v_pos_stride;
      std::memmove(
          dst,
          src,
          num_positions * v_pos_stride * sizeof(T));
    }
  }
}
