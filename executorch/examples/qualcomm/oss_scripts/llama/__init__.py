...
LLM_VARIANT_ARCHS: Dict[str, LlamaModel] = {
    ...
    "internvl3_8b": LlamaModelWithoutEmbedding,
...
@dataclass(init=False, frozen=True)
class InternVL3_8B(LLMModelConfig):
    repo_id: str = "OpenGVLab/InternVL3-8B-hf"
    params_path: str = os.path.join(
        BASE_DIR, "../../../models/internvl3/8b_config.json"
    )
    convert_weights = convert_internvl3_weights
    transform_weight = False
    instruct_model = True
    num_sharding = 8  # 8B exceeds 4GB per-context HTP limit; shard to reduce per-shard memory
    masked_softmax = True
    seq_mse_candidates = 0
    r1 = False
    r2 = False
